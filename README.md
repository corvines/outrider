...
This installs `~/.local/bin/outrider` without administrator privileges. Add `~/.local/bin` to `PATH` if needed. Running `install` again upgrades an Outrider-owned installation; `outrider uninstall` removes it safely.
